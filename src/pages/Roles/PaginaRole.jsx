...
export const PaginaRole = () => {
  ...
  const { roles, loading, error, metadata } = useSelector(
    (state) => state.roles,
  );
  ...
  useEffect(() => {
    if (roles && roles.length > 0) {
      const datosFiltrados = roles.filter((role) =>
        role.nombre.toLowerCase().includes(filtroBusqueda.toLowerCase()),
      );
  ...
  const metadataParaForm =
    metadataProcesada.length > 0 ? metadataProcesada : metadataFallbackRoles;
            ...
            return (
              <>
                <BotonEditar
                  ...
                  formData={rowData}
                />
                <BotonEliminar nombreBoton="Eliminar Rol" formData={rowData} />
              </>
            );
          }
