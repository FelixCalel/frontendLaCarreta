...
export const tablaPedidos = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(`${BASE_URL}/pedido/todos`);
...
export const addNewPedido = createAsyncThunk(
  ...
  async (newPedido) => {
    const response = await axios.post(`${BASE_URL}/pedido/create`, newPedido);
...
export const deletePedido = createAsyncThunk(
  ...
  async (id) => {
    await axios.delete(`${BASE_URL}/pedido/eliminar/${id}`);
    return id;
  }
);
...
export const updatePedido = createAsyncThunk(
  ...
  async (pedido) => {
    const response = await axios.put(`${BASE_URL}/pedido/actualizar/${pedido.id}`, pedido);
    return response.data;
  }
);
