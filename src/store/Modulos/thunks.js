...
export const fetchModulos = createAsyncThunk('modulos/fetchModulos',
  async (_, thunkAPI) => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/asignarRMOP/modulosPermisos/${auth.uid}`);
...
export const createModulo = createAsyncThunk('modulos/createModulo',
  async (moduloData, thunkAPI) => {
    try {
      ...
      const response = await axios.post(`${BASE_URL}/modulos/crear`, modulo);
      return response.data;
    } catch (error) {
      // Manejo de errores
      return thunkAPI.rejectWithValue(error.response ? error.response.data : error.message);
...
export const updateModulo = createAsyncThunk(
  ...
  async (moduloData, { rejectWithValue }) => {
      try {
          const response = await axios.put(`${BASE_URL}/modulos/update`, moduloData);
          return response.data;
      } catch (error) {
...
export const deleteModulo = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
      try {
          const response = await axios.delete(`${BASE_URL}/modulos/eliminar/${id}`);
          return response.data;
      } catch (error) {
          if (error.response && error.response.data) {
              return rejectWithValue(error.response.data);
