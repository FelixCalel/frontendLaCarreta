...
import { Button, Input, FormLabel, Box, Switch, Grid, GridItem } from "@chakra-ui/react";
import iconCatalog from '../../../Iconos/IconCatalog';

export const FormularioNuevoEditar = ({ formData, onSubmit, onClose, metadata }) => {
    ...
    useEffect(() => {
        if (formData) {
            setFormValues({
                ...formData,
                estado: formData.estado === "Activo" || formData.estado === true ? true : false,  // Acepta "Activo" o un valor booleano
            });
    ...
    return (
        <Box>
            ...
            <Box mt={6}>
                ...
                variant="outline"
                size="md"
                borderRadius="full" onClick={handleSubmit}>
                    Guardar Cambios
                </Button>
                <Button ml={4}  colorScheme="teal"
                variant="outline"
                size="md"
                borderRadius="full" onClick={onClose}>
                    Cancelar
                </Button>
            </Box>
        </Box>
