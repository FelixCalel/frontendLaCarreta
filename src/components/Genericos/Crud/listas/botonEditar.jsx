...
import { ModalEditGeneric } from '../Modal/ModalEditGeneric.jsx';
...
export const BotonEditar = ({ nombreBoton, metadata, formData }) => {
    ...
    const handleSubmit = (formData) => {
        ...
        switch (nombreBoton) {
            case 'Editar Módulo':
                dispatch(updateModulo(updatedFormData))
                    .then((result) => {
            ...
            case 'Editar Opción':
                dispatch(updateOpciones(updatedFormData))
                    .then((result) => {
                        if (result.meta.requestStatus === 'fulfilled') {
                            setIsAlertOpen(true);
                            onClose();
            ...
            case 'Editar Permiso':
                dispatch(updatePermiso(updatedFormData))
                    .then((result) => {
                        if (result.meta.requestStatus === 'fulfilled') {
                            setIsAlertOpen(true);
                            onClose();
            ...
            case 'Editar Rol':
                dispatch(updateRol(updatedFormData))
                    .then((result) => {
                        if (result.meta.requestStatus === 'fulfilled') {
                            setIsAlertOpen(true);
                            onClose();
    ...
    return (
        <>
            ...
            >
                Editar
                <FaEdit fontSize="15px" style={{ marginLeft: '8px' }} /> {/* Margen izquierdo para separar el icono */}
            </Button>
